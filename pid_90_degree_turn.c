   ...
   // float leftMotorSpeed = 1;
    ...
    if (clientID!=-1){
		...
		while (simxGetConnectionId(clientID)!=-1){
            ...
            if(sensor_value[0]<0.4 && sensor_value[1]<0.4 && sensor_value[2]<0.4 && sensor_value[3]>0.4)			//left turn detected
			{           
		    ...
		    if(sensor_value[3]<0.4 && sensor_value[1]<0.4 && sensor_value[2]<0.4 && sensor_value[0]>0.4)			//right turn detected
			{        
