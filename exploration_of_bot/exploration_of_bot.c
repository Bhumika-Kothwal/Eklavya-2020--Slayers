...
int junc_check();
int old_node(int i, int node_type, int turn_stored[],int t);
...
int travel(int z,int cordinate[][2],int d, int node_type)	//function for retracing
{
	float *sensor_values;
	int v;
	float turn_speed=1.5;
	int speed=0;
	int x2,y2,x1,y1,c = 0,count =0 ,l;
	int ab=0;
	x1=cordinate[z][0];
	y1=cordinate[z][1];
	y2=cordinate[d][1];		// d-> index of coordinate in coordinate array
	x2=cordinate[d][0];
			...
			{	//reading sensors
				...
				else
				{
					if((sensor_values[0]>0.7 && sensor_values[1]>0.7 && sensor_values[2]>0.7 && sensor_values[3]>0.7) || (sensor_values[0]>0.7 && sensor_values[1]<0.7 && sensor_values[2]<0.7 && sensor_values[3]>0.7))
					{
						...
						all_cordinate[all_cordinate_index][1] = y2;
						all_cordinate_index+=1;
						cordinate_index = findnow(x2 , y2 , total_points);
						array[g]=cordinate_index;
						g++;
